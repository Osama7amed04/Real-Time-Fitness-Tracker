...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    while cap.isOpened():
        ...
        try:
            ...
            if knee_angle > START_ANGLE:
                if state["stage"] == "going_target":
                    state["error_msg"] = "HALF SQUAT! GO LOWER!"
                ...
                if state["stage"] == "going_start":
                    state["error_msg"] = "FAIlED REP! STAND UP FULLY!"
                    state["error_time"] = time.time()
                    play_wrong_audio()
                state["stage"] = "target"
            ...
            cv2.putText(image, "FRONT SQUAT", (20, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 200, 0), 2)
        ...
        cv2.imshow("FRONT SQUAT TRACKER", image)
