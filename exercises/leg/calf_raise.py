...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    while cap.isOpened():
        ...
        try:
            ...
            if ankle_angle < START_ANGLE + 10:
                if state["stage"] == "going_target":
                    state["error_msg"] = "HALF RAISE! UP ON TOES!"
            ...
            elif ankle_angle > TARGET_ANGLE:
                if state["stage"] == "going_start":
                    state["error_msg"] = "FAILED REP! HEELS DOWN!"
                    state["error_time"] = time.time()
                    play_wrong_audio()
                state["stage"] = "target"

            elif START_ANGLE + 10 <= ankle_angle <= TARGET_ANGLE:
                if state["stage"] == "start":
                    state["stage"] = "going_target"
                elif state["stage"] == "target":
                    state["stage"] = "going_start"

            cv2.putText(image, "CALF RAISE", (20, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 200, 0), 2)
        ...
        cv2.imshow("CALF RAISE TRACKER", image)
